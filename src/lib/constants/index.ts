export const APPWRITE_PROJECT_ID = process.env.NEXT_PUBLIC_APPWRITE_PROJECT_ID!;
export const APPWRITE_PROJECT_NAME =
  process.env.NEXT_PUBLIC_APPWRITE_PROJECT_NAME!;
export const APPWRITE_ENDPOINT = process.env.NEXT_PUBLIC_APPWRITE_ENDPOINT!;

// Appwrite Database and Collections
export const DATABASE_ID = process.env.NEXT_PUBLIC_DATABASE_ID!;
export const TRADE_PLANS_TABLE_ID =
  process.env.NEXT_PUBLIC_TRADE_PLANS_TABLE_ID!;
export const WATCHLISTS_TABLE_ID = process.env.NEXT_PUBLIC_WATCHLISTS_TABLE_ID!;
export const WATCHLIST_ITEMS_TABLE_ID =
  process.env.NEXT_PUBLIC_WATCHLIST_ITEMS_TABLE_ID!;
